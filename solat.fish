...
# Print the results
echo -n "Solat times for today $hijri_date $hijri_month $hijri_year or "
date "+%d %B %Y"
...
set times (curl -s "https://mpt.i906.my/mpt.json?code=jhr-2&filter=1" | jq -r '.response.times[] | (.+28800) | strftime("%H:%M")')
